...
class ToTensor(object):
    ...
    def __call__(self, sample):
        ...
        return {'image': image, 'depth': depth, 'mask': mask, 'dataset': "ExpNoise"}
...
class ExpNoise(Dataset):
    ...
    def crawl_folders(self):
        for tl in self.textures:
            imgs = sorted(Path(tl).files('FrameBuffer_*.*g'))
            depths = sorted(Path(tl).files('Depth_*.*g'))
            
            if self.mode == "train":
                self.image_files.extend(imgs[:-10])
                self.depth_files.extend(depths[10:])
            else:
                self.image_files.extend(imgs[:-10:10])
                self.depth_files.extend(depths[10::10])
...
def get_ExpNoise_loader(data_dir_root, resize_shape, mode="val", batch_size=1, **kwargs):
    dataset = ExpNoise(data_dir_root, resize_shape, mode)
    return DataLoader(dataset, batch_size, **kwargs)


if __name__ == "__main__":
    loader = get_ExpNoise_loader(
        data_dir_root="/mnt/data/tqy/Exp_NoisyLabels/", resize_shape=[384, 384])
