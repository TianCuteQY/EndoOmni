...
class SCARED(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        try:
            ...
            depth_path = self.depth_files[idx+1]
            image = Image.open(image_path).convert('RGB')
            depth = np.array(Image.open(depth_path)) / 255.0
            depth = depth[..., None]
...
def get_SCARED_loader(data_dir_root, resize_shape, mode="train", batch_size=1, **kwargs):
...
if __name__ == "__main__":
    loader = get_SCARED_loader(
        data_dir_root="/mnt/samba_share/SCARED/out", resize_shape=[384, 384])
